...
const allTokensExist = computed(() => {
  const selectedAccount = props.selectedAccount;
  if (!selectedAccount) {
    return false; // If selectedAccount is not set, tokens cannot exist
  }

  const blockchains = appBlockchains.blockchains;

  return blockchains.every((blockchain) => {
    // Check if all tokens in the current blockchain exist in at least one account
    return blockchain.tokens.every((blockchainToken) => {
      // Check if the token exists in at least one account
      return Object.values(
        props.accounts[selectedAccount.publicKey].tokenAccounts,
      ).some((account) => {
        return Object.values(account.metaData).some(
          (name) => name == blockchainToken.metaData.name,
        );
      });
    });
  });
...
<template>
  <div class="banking-dashboard banking-dashboard-v2 mt-5">
    <div class="standard-onboarding">
      ...
      <div class="onboarding-wrap">
        <div class="onboarding-wrap-inner">
          ...
          <div v-if="!allTokensExist" class="onboarding-card">
            <h3>{{ t("pages.wallet.manage.newTokenAccount") }}</h3>
            <div class="button-wrap">
              <VButton
                color="primary"
                outlined
                ...
                @click="
                  () => {
                    modal = true;
                    active = 'createTokenAccount';
                  }
                "
              >
                {{ t("action.wallet.newTokenAccount") }}
              </VButton>
            </div>
          </div>
    ...
    >
      <template #content>
        <CreateTokenAccount
          v-if="active == 'createTokenAccount' && !allTokensExist"
          @close="closeModal"
