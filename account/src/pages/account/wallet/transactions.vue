...
const selectedPublicKey = ref(props.selectedPublicKey);
...
const handleGetTransactions = async () => {
  ...
  transactions.data = [];
  await api
    .get("wallet/transactions/get", {
      params: { publicKey: props.selectedPublicKey },
    })
    .then((response) => {
...
const handleCancel = async (signature: string) => {
  const getTransaction = transactions.data.find((transactionGroup) => {
    // Check if the transaction group is an array
    if (Array.isArray(transactionGroup)) {
      // Check the first signature in the group
...
watchEffect(() => {
  if (selectedPublicKey.value != props.selectedPublicKey) {
    currentPage.value = 1;
    filters.value = "";
    handleGetTransactions();
    selectedPublicKey.value = props.selectedPublicKey;
  }
});
          ...
          <TransitionGroup name="list" tag="div" class="flex-list-inner">
            ...
            >
              ...
              >
                ...
                <VTag
                  v-else-if="transaction[0].data?.status == 'cancelled'"
                  color="warning"
                  rounded
                >
                  Canceled
                ...
                <VTag v-else color="success" rounded> Confirmed </VTag>
              </VFlexTableCell>
              <VFlexTableCell :column="{ grow: true }" data-th="Label">
              ...
              <VFlexTableCell :column="{ align: 'end' }">
                <TransactionWalletTableDropdown
                  :selectedAccountPubKey="props.selectedAccount.publicKey"
                  :transaction="transaction"
                  @view="handleView(transaction[0].signature)"
                  @confirm="handleConfirm(transaction[0].signature)"
                  @cancel="handleCancel(transaction[0].signature)"
              ...
              <div class="box-content">
                ...
                <ConfirmEscrow
                  v-if="
                    ...
                    transactionData.data.status == 'initialized'
                  "
